...
export function createMapping({ components, module, mapping, filter, dts }) {
  const importMapping = {};
  ...
  makeArray(components).forEach(async comp => {
    ...
    for (let componentPath of componentPaths){
      traverse(componentPath, filter, name => {
        let moduleName = getModuleName(componentPath, name, flat, prefix);
        let modulePath = normalizePath(path.relative(moduleName, name));
        usedComponents.push({
          "component": moduleName,
          "path": modulePath.replace("../src", "."),
          "name": "default"
        });
      });
    }
  ...
  generateDTS(usedComponents, dts)
...
export function generateDTS(components, output){
  components = components.filter((v,i,a)=>a.findIndex(v2=>['component','path'].every(k=>v2[k] ===v[k]))===i)

  let dts = "// Generated by 'sveltekit-autoimport'\nexport {}\ndeclare global {\n"

  components.forEach(c=>{
    dts += `const ${c.component}: typeof import('${c.path}')['${c.name}']\n`
  })
